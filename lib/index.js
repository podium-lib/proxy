    ...
    middleware() {
        ...
        router.all(
            ...
            (req, res, next) => {
                ...
                const extras = req.url.substring(
                    `${this.resourceMountPath}/${podletName}/`.length
                );

                if (resource) {
                    const uri = utils.uriBuilder(
                        '/public',
                        resource.uri,
                        extras
                    );
                    this.logger.debug(
                        `Proxing ${req.method} ${req.url} to fragment at ${uri}`
                    );
                    this.proxy.request(uri, req, res);
                } else {
                    this.logger.warn(
                        `${podletName} is not a known podlet resource`
                    );
                    next();
                }
            }
