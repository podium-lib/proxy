...
*/
    ...
    register(manifest = {}) {
        ...
        // Hande that a @podium/podlet instance is passed in
        if(Object.prototype.toString.call(manifest) === '[object PodiumPodlet]') {
            obj = JSON.stringify(manifest);
        }
...
// TODO: Infinity here is a workaround for global Intinity in the cache not working
        this.registry.set(validatedManifest.value.name, validatedManifest.value, Infinity);
    }

// TODO: path should always start with a / and not end with one. Check this and adjust stuff!!
    middleware(path = '/podium-resource') {
        const keys = [];
        ...
        return (req, res, next) => {
            ...
            if (match) {
                ...
                const entries = this.registry.entries().filter(item => {
                    return (item.name === params.podiumPodletName) ;
                });
                ...
                if (putils.uriIsRelative(target)) {
                    next();
                    return;
                }
