...
*/
    ...
    middleware(mountPathname = `/${PROXY_PATHNAME}`) {
        ...
        return (req, res, next) => {
            ...
            // Compensate for proxy being mounted on a express.js route
            // This handle that the mountPathname is to be found further down
            // in a pathname. Ex: /a/b/{mountPathname}/
            if (req.baseUrl) {
                pathname = pathname.substring(req.baseUrl.length);
            }

            const match = parser.exec(pathname);
